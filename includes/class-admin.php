...
readonly class CachemanAdmin {
	...
	public function sanitize_settings( array $input ): array {
		...
		if ( empty( $extra_domains_input ) ) {
			...
			$valid_domains              = array_filter(
				$domains,
				function ( string $domain ): bool {
					if ( empty( $domain ) ) {
						return false;
					}
													...
													<div class="zw-cacheman-details-content">
														...
														if ( $log_content ) {
															...
															if ( $total_lines > 100 ) {
																...
																printf( esc_html__( 'Showing last 100 of %d entries', 'zw-cacheman' ), absint( $total_lines ) );
																echo '</p>';
															}
															?>
										...
										<?php if ( $next_run ) : ?>
											...
											if ( $seconds > 0 ) {
												...
												if ( $minutes > 0 ) {
													...
													printf( esc_html__( 'Next run in %d min', 'zw-cacheman' ), absint( $minutes ) );
												} else {
													/* translators: %d: seconds until next run */
													printf( esc_html__( 'Next run in %ds', 'zw-cacheman' ), absint( $seconds ) );
												}
											} else {
