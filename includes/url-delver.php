...
readonly class CachemanUrlDelver {
	...
	public function get_high_priority_purge_items( \WP_Post $post ): array {
		...
		if ( $post_type_obj && $post_type_obj->show_in_rest ) {
			...
			$urls[] = [
				'url'  => rest_url( 'wp/v2/' . $rest_base . '/' . $post->ID ),
				'type' => PurgeType::File,
			];

			// Post type collection endpoint.
			$urls[] = [
				'url'  => rest_url( 'wp/v2/' . $rest_base ),
				'type' => PurgeType::File,
			];
		}
	...
	public function get_low_priority_purge_items( \WP_Post $post ): array {
				...
				foreach ( $terms as $term ) {
					...
					if ( ! is_wp_error( $term_link ) ) {
						...
						if ( $tax_obj && $tax_obj->show_in_rest ) {
							...
							$urls[] = [
								'url'  => rest_url( 'wp/v2/' . $rest_base . '/' . $term->term_id ),
								'type' => PurgeType::File,
							];
							$urls[] = [
								'url'  => rest_url( 'wp/v2/' . $rest_base ),
		...
		if ( post_type_supports( $post->post_type, 'author' ) ) {
			...
			if ( $author_url ) {
				...
				$urls[] = [
					'url'  => rest_url( 'wp/v2/users/' . $post->post_author ),
					'type' => PurgeType::File,
				];
			}
		}
		...
		$urls[] = [
			'url'  => rest_url( 'wp/v2/taxonomies' ),
			'type' => PurgeType::File,
		];

		$urls = array_merge( $urls, $this->get_sitemap_urls_for_post_type( $post->post_type ) );
	...
	public function get_low_priority_term_purge_items( \WP_Term $term, string $taxonomy ): array {
		...
		$urls = array_merge( $urls, $this->get_sitemap_urls_for_taxonomy( $taxonomy ) );
	...
	public function get_deleted_term_purge_items( int $term_id, string $taxonomy ): array {
		...
		if ( $tax_obj && $tax_obj->show_in_rest ) {
			...
			$urls[] = [
				'url'  => rest_url( 'wp/v2/' . $rest_base . '/' . $term_id ),
				'type' => PurgeType::File,
			];
	 ...
	 */
	private function get_sitemap_urls_for_post_type( string $post_type ): array {
	 ...
	 * @return array<array{url: string, type: PurgeType}>
	 */
	private function get_sitemap_urls_for_taxonomy( string $taxonomy ): array {
		if ( ! $this->sitemap_provider || ! $this->sitemap_provider->is_active() ) {
			return [];
		}
